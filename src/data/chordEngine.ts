...
function parseSymbol(
  symbol: string,
): { root: string; quality: Quality; approx: boolean } | null {
  const m = symbol.match(/^([A-G][#b]?)(.*)$/)
  if (!m) return null
  const root = m[1]
  if (!(root in PITCH_CLASS)) return null
  // Drop slash bass — we play the base chord shape.
  const [rest, bass, extra] = m[2].split('/')
  if (extra !== undefined || (bass !== undefined && !/^[A-G][#b]?$/.test(bass)))
    return null
  // Require a whole chord suffix: words such as ‘Goodbye’ are lyrics, not G diminished.
  if (
    !/^(?:(?:maj|min|m|M|dom|dim|aug|sus|add|Δ|°|o|\+|-)?\d{0,2}(?:(?:add|sus|[#b])\d{1,2})*)$/.test(
      rest,
    )
  )
    return null
...
export function resolveChord(symbol: string): ResolvedChord | null {
  ...
  if (dictKey in CHORDS) {
    return {
      symbol,
      shape: { ...CHORDS[dictKey], name: symbol },
      approx: false,
    }
  }
